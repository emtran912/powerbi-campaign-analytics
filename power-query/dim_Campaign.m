let
    ...
    #"Select Columns" = Table.SelectColumns(#"Updated Types",{"CampaignName", "Campaign Category"}),
    #"Remove Duplicates" = Table.Distinct(#"Select Columns"),
    #"Add Campaign Key" = Table.AddColumn(
        #"Remove Duplicates", 
        "Campaign Key", 
        each Text.Remove(Text.Upper([CampaignName]), {" ", "-", "."})
    ),
    #"Remove Duplicates Key" = Table.Distinct(#"Add Campaign Key", {"Campaign Key"}),
    #"Add Campaign ID" = Table.AddIndexColumn(#"Remove Duplicates Key", "Campaign ID", 1, 1, Int64.Type)
in
    #"Add Campaign ID"